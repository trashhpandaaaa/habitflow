...
export async function GET() {
  try {
    ...
    for (const habit of habits) {
      // Count actual completions for this habit using Clerk userId
    ...
    return NextResponse.json({
      message: 'Successfully recalculated completion counts',
      updates,
      totalHabitsUpdated: updates.length
    });
    
  } catch (error) {
    console.error('Fix completion counts error:', error);
    return NextResponse.json(
